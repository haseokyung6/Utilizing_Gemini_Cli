# streamlit run Starbucks_WebScraping/dashboard.py
...
@st.cache_data
def load_data():
    df = pd.read_csv('Starbucks_WebScraping\data\starbucks_AIdata.csv')
    # 'gugun_nm' 컬럼의 결측치를 '정보없음'으로 채웁니다.
    df['gugun_nm'] = df['gugun_nm'].fillna('정보없음')
    # 'open_dt' 컬럼을 datetime 객체로 변환합니다.
...
# 사이드바 설정
st.sidebar.title('스타벅스 매장 정보')
# st.sidebar.markdown('**분석/시각화 메뉴 이동은 메인 화면의 탭을 이용해주세요.**')
...
with tab1:
    ...
    st.subheader('연도별 매장 오픈 수')
    df['open_year'] = df['open_dt'].dt.year
    st.bar_chart(df['open_year'].value_counts().sort_index())
...
with tab3:
    ...
    for idx, row in map_df.iterrows():
        # 검색된 매장은 빨간색으로 강조
        if store_name_search and store_name_search.lower() in row['s_name'].lower():
